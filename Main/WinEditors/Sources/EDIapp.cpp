...
char *EDI_gpz_SlashP  = nullptr;
char *EDI_gpz_SlashPG = nullptr;
#	ifdef JADEFUSION
...
HWND EDI_gh_TopMenuMode  = nullptr; /* Top menu selection mode (menu) */
...
/*
 ...
 =======================================================================================================================
 */
void EDI_cl_EnterWnd::DisplayMessage( const char *_psz_Msg )
{
	/*~~~~~~~~~~~~~~~~~*/
	CDC *pdc;
	CRect o_Rect;
...
/*
 ...
 =======================================================================================================================
 */
void EDI_cl_ToolTipWnd::Display()
{
	/*~~~~~~~~~~~~~~~~~~~~~~*/
	CDC *pdc;
...
EDI_cl_EnterWnd *EDI_gpo_EnterWnd = nullptr;
...
#		endif
	...
	if ( EDI_gb_NoVerbose )
	{
		...
		{
			SetFilePointer( EDI_gh_ErrorFile, 0, nullptr, FILE_END );
...
#	endif// XML_CONV_TOOL
	...
	mb_RunEngine        = FALSE;
	mpo_LastWnd         = nullptr;
	mpo_LastView        = nullptr;
	mpo_FocusWndWhenDlg = nullptr;
	...
	if ( LOA_gb_SpeedMode )
	{
		auto *po_Out = ( EOUT_cl_Frame * ) M_MF()->po_GetEditorByType( EDI_IDEDIT_OUTPUT, 0 );
		if ( po_Out )
		{
...
#	endif
	...
	mh_HookKeyboard = SetWindowsHookEx( WH_KEYBOARD, x_KeyboardHook, nullptr, AfxGetThread()->m_nThreadID );

	/* Create hook for mouses messages */
	mh_HookMouse = SetWindowsHookEx( WH_MOUSE, x_MouseHook, NULL, AfxGetThread()->m_nThreadID );

	while ( ::PeekMessage( &( GE_MFC_MSG ), nullptr, NULL, NULL, PM_REMOVE ) )
	{
	...
	EDI_gpo_EnterWnd2->Create( nullptr, NULL, WS_POPUP, CRect( 0, 0, 1, 1 ), mpo_MainFrame, NULL );
...
BOOL EDI_cl_App::PumpMessage( void )
{
	...
	if ( ( GE_MFC_MSG ).message != WM_KICKIDLE && !PreTranslateMessage( &( GE_MFC_MSG ) ) )
	{
		__try
		{
			_Try_ ::TranslateMessage( &( GE_MFC_MSG ) );
			::DispatchMessage( &( GE_MFC_MSG ) );
			_Catch_
			        _End_
		}
...
int EDI_cl_App::Run( void )
{
	...
	__try
	{
		...
		for ( ;; )
		{
			...
			} while ( ::PeekMessage( &( GE_MFC_MSG ), nullptr, NULL, NULL, PM_NOREMOVE ) );
		}
	}
...
/*
 ...
 =======================================================================================================================
 */
EDI_cl_BaseFrame *EDI_cl_App::GetFocusedEditor()
{
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	CWnd *po_Wnd, *po_Wnd1;
	EDI_cl_BaseView *po_View;
...
/*
 ...
 =======================================================================================================================
 */
void EDI_cl_App::SetFocusPaneColor()
{
	/*~~~~~~~~~~~~~~~~~~~~~~*/
	CWnd *po_Wnd, *po_Wnd1;
	int i;
	...
	if ( po_Wnd != mpo_LastWnd )
	{
		...
		if ( po_Wnd1 != mpo_LastView )
		{
			if ( po_Wnd1 )
			{
				po_Wnd1->RedrawWindow( nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASENOW | RDW_FRAME );
				if ( ( ( EDI_cl_BaseView * ) po_Wnd1 )->po_GetActivatedEditor() )
				{
					if ( ( ( EDI_cl_BaseView * ) po_Wnd1 )->po_GetActivatedEditor()->mpo_Menu != nullptr )
			...
			if ( mpo_LastView )
			{
				mpo_LastView->RedrawWindow( nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASENOW | RDW_FRAME );
				if ( ( ( EDI_cl_BaseView * ) mpo_LastView )->po_GetActivatedEditor() )
				{
					if ( ( ( EDI_cl_BaseView * ) mpo_LastView )->po_GetActivatedEditor()->mpo_Menu != nullptr )
...
BOOL EDI_cl_App::b_DispatchKey( USHORT _uw_Key, BOOL _b_Pressed, BOOL _b_Glob )
{
	...
	if ( _b_Glob || ( M_MF()->mpo_MaxView && M_MF()->mst_Desktop.b_VeryMaximized ) )
	{
		...
		while ( pos )
		{
			...
			if (
			        ( po_List->mpo_Editor ) && ( po_List->mpo_Editor != po_Focus ) && ( po_List->mpo_Editor->mb_IsActivate ) && ( po_List->mpo_Editor->mb_Persist ) )
			{
...
LRESULT CALLBACK x_KeyboardHook( int code, WPARAM wParam, LPARAM lParam )
{
	...
	if ( wParam == VK_SPACE )
	{
		...
		if ( po_Wnd )
		{
			...
			{
				if (
				        ( GetAsyncKeyState( VK_SHIFT ) >= 0 ) && ( GetAsyncKeyState( VK_CONTROL ) >= 0 ) && ( GetAsyncKeyState( VK_MENU ) >= 0 ) ) return TRUE;
			}
		}
	}
