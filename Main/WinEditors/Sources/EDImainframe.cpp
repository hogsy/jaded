...
if (!EDI_gb_DontCheckVersion)
{
    if(editorVersionCheck() == FALSE)  
    {
        return FALSE;
    }
}
...
=======================================================================================================================
*/
BOOL EDI_cl_MainFrame::editorVersionCheck()
{
    FILE    *f;
    char	asz_VersionFile[EDI_VERSION_FILENAME_LENGTH];
    char	asz_Version[EDI_VERSION_LENGTH];
    int     iVersion;

    strcpy(asz_VersionFile, "\\\\ubisoft_mtl\\projects\\KingKong\\VersionProduction\\Prod\\PS2\\Editor\\Version_Number.txt");    
    f = L_fopen(asz_VersionFile, "r");    
    if(f)
    {
        L_fread(asz_Version, EDI_VERSION_LENGTH, 1, f);
        iVersion = atoi(asz_Version);
        fclose(f);

        if (iVersion != BIG_Cu4_MontrealAppVersion)
        {
            char errMsg[EDI_ERR_MSG_LENGTH];
            //sprintf(errMsg, "Bad Editor version! (Current : %i, Yours : %i)\n Do you realy want to continue?", iVersion, BIG_Cu4_MontrealAppVersion);
            sprintf(errMsg, "Bad Editor version! (Current : %i, Yours : %i)\n Do you want to upgrade to new version?", iVersion, BIG_Cu4_MontrealAppVersion);
            if(MessageBox(errMsg, "Bad Version", MB_ICONQUESTION | MB_YESNO) == IDYES)
            {
                // Install editor and quit
                PROCESS_INFORMATION pi;
                STARTUPINFO			si;
                BOOL				retVal = FALSE;
                char szCmdLine[ 512 ];

                sprintf( szCmdLine, "\\\\ubisoft_mtl\\projects\\KingKong\\VersionProduction\\Tools\\sync_editor.exe" );
                ...
                {
                    MessageBox( "Unable to install engine!", "Error", MB_OK | MB_ICONEXCLAMATION );
                }

                return FALSE;
            }         

            sprintf(errMsg, "Do you really want to continue (Perforce will be disable)?", iVersion, BIG_Cu4_MontrealAppVersion);
            if(MessageBox(errMsg, "Continue?", MB_ICONQUESTION | MB_YESNO) == IDNO)
            {
                return FALSE;
            }         
    ...
    {       
        if(MessageBox("Error accessing version, please contact your Data Manager.\nDo you realy want to continue?", "Error", MB_ICONQUESTION | MB_YESNO) == IDNO)
        {
            return FALSE;
        }
