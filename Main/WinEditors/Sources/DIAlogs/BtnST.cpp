...
void CButtonST::PreSubclassWindow() 
{
	...
	if (nBS & BS_CHECKBOX || m_nTypeStyle == BS_AUTOCHECKBOX || m_nTypeStyle == BS_RADIOBUTTON || m_nTypeStyle == BS_AUTORADIOBUTTON) m_bIsCheckBox = TRUE;
...
BOOL CButtonST::PreTranslateMessage(MSG* pMsg) 
{
	...
	res = CButton::PreTranslateMessage(pMsg);
	return res;
...
void CButtonST::OnEnable(BOOL bEnable) 
{
	...
	if (GetFocus() == this && bEnable == FALSE)	
	{
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	CBrush	br2(GetSysColor(COLOR_ACTIVECAPTION));
	CBrush *pbr = pDC->SelectObject(m_bIsDisabled ? &br1 : m_nCheck ? &br2 : &br);
	...
	// Draw the icon
	if(m_csIcons[0].hIcon != GetIcon()) SetIcon(GetIcon());
	...
	if(!m_csBitmaps[0].hBitmap && GetBitmap()) SetBitmaps(GetBitmap(), RGB(192,192,192));
...
void CButtonST::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if(m_bIsRealCheckBox)
		align = DT_LEFT | DT_VCENTER | DT_SINGLELINE;
	...
	else if(!m_bIsRealCheckBox)
	{
		pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | align | DT_CALCRECT);
	...
	if (m_bIsDisabled && !m_bIsGrpBox)
	{
		pDC->SetTextColor(::GetSysColor(COLOR_3DSHADOW));
		pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | align);
	} 
	else
	{
		if(m_bIsGrpBox)
		{
			...
			pDC->DrawText(str, rpCaption, DT_WORDBREAK | align);
			return;
		}
		else if(m_bIsXP && m_nCheck)
		{
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	...
	if(m_bIsXP)
	{
		...
		if(m_bMouseOnButton || (m_bIsDefault && !m_bIsCheckBox)) rect.InflateRect(CSize(-1, -1));
		if(m_bIsDisabled)
		{
		...
		{
			pDC->FillSolidRect(rect, m_nCheck ? GetSysColor(COLOR_ACTIVECAPTION) : 0x00FFFFFF);
		}
		else
		{
			crBase = 0x00FFFFFF;
			for (int i=rect.top; i<rect.Height()+2; i++) 
			{
				rct.SetRect (rect.left, i, rect.Width()+2, i + 1);
				j = ((i * 63) / rect.Height());
				crColor = RGB((crBase>>0)-(j/3), (crBase>>8)-(j/3), (crBase>>16)-(j/5));
				pDC->FillSolidRect(&rct, crColor);
			}
		}
...
DWORD CButtonST::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	...
	CPen pen3DDKShadow(PS_SOLID, 1, GetSysColor(COLOR_3DDKSHADOW)); // Black
	CPen penXPInside(PS_SOLID, 0, GetSysColor(COLOR_ACTIVECAPTION)); // Black
	...
	if(m_bIsGrpBox)
	{
		...
		poldb = (HBRUSH) pDC->SelectObject(brush);
		pDC->SetBkMode(TRANSPARENT);
		pDC->RoundRect(pRect, CPoint(ROUNDRECT, ROUNDRECT));
	...
	if (m_bIsPressed)
	{
		if(m_bIsXP)
		{
			CPen* pOldPen = pDC->SelectObject(m_bMouseOnButton ? &penXPInside : &penBtnShadow);
			...
			if (m_bMouseOnButton && m_bDrawBorder)
			{
				...
				pDC->RoundRect(rect, CPoint(ROUNDRECT, ROUNDRECT));
			}
	...
	else // ...else draw non pressed button
	{
		if(m_bIsXP)
		{
			...
			if (m_bMouseOnButton && m_bDrawBorder)
			{
				CPen* pOldPen = pDC->SelectObject(&penXPInside);
				CRect rect = pRect;
				pDC->RoundRect(pRect, CPoint(ROUNDRECT, ROUNDRECT));
				rect.InflateRect(CSize(-1, -1));
				pDC->RoundRect(rect, CPoint(ROUNDRECT, ROUNDRECT));
			...
			{
				CPen *pen, *pOldPen;
				pen = m_bIsDisabled ? &penBtnShadow : &pen3DDKShadow;
				pOldPen = pDC->SelectObject(pen);
				pDC->RoundRect(pRect, CPoint(ROUNDRECT, ROUNDRECT));
				if(m_bIsDefault && !m_bIsCheckBox)
				{
					CRect rect = pRect;
					rect.InflateRect(CSize(-1, -1));
					pDC->RoundRect(rect, CPoint(ROUNDRECT, ROUNDRECT));
