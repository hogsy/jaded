...
m_ThClOver(RGB(32,205,0)),//205,32,0)),//thumb+focus
m_ThClOut(RGB(10, 110, 10)),//::GetSysColor(COLOR_3DDKSHADOW)),//thumb+no focus
m_ThClPre(RGB(32,205,0)),//thumb+scrolling
m_ProgressBarColor(RGB(151,151,151)),
m_ThClDef(m_ThClOut),
m_ProgressStep(10),
m_bBorders(FALSE)
{
    ...
    m_ProRange.iLow = 0;
	m_ProRange.iHigh = 10;
    m_ProgressPos = m_ProRange.iHigh;
}
...
HRESULT CProSliderCtrl::DrawSliderChannel(LPNMCUSTOMDRAW pNMCD)
{
	...
	case TRUE:
		...
		{
            if(m_ProgressPos == m_ProRange.iHigh)
                rr=crect.Height();
            else
...
__inline void CProSliderCtrl::RecursiveRect(CDC *pDC, CRect rc, COLORREF nClr, BOOL First/*=FALSE*/)
{
    ...
    if(First) this->GetStyle() & TBS_VERT ? rc.InflateRect(1,0) : rc.InflateRect(0,1);
    pDC->RoundRect(rc,CPoint(2,2));
...
void CProSliderCtrl::OnKillFocus(CWnd* pNewWnd) 
{
...
void CProSliderCtrl::OnSetFocus(CWnd* pOldWnd) 
{
