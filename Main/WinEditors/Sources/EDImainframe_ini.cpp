...
#	endif
	...
	o_Name = EDI_Csz_Ini + CString( "/" ) + _psz_IniFile + CString( EDI_Csz_ExtIni );

	std::vector< uint8_t > buf;
	if ( jaded::filesystem.ReadFileByName( ( LPCSTR ) o_Name, &buf ) )
	{
		memcpy( _puc_IniStruct, &buf[ 0 ], buf.size() );
		return;
	}
	...
	*psz_Temp = 0;
	...
	o_Name = EDI_Csz_Ini + CString( "/" ) + asz_Name + EDI_Csz_ExtIniDef + EDI_Csz_ExtIni;
	if (jaded::filesystem.ReadFileByName((LPCSTR)o_Name, &buf))
	{
		memcpy( _puc_IniStruct, &buf[ 0 ], buf.size() );
	}
...
void EDI_cl_MainFrame::SynchronizeEditorsData( BOOL _b_Sens )
{
	...
	if ( _b_Sens )
	{
		...
		*strrchr( az, '\\' ) = 0;
		...
		_Catch_
		        _End_ if ( EDI_gpo_EnterWnd )
		                EDI_gpo_EnterWnd->DisplayMessage( "Synchronizing EditorsData : OK" );
		else LINK_PrintStatusMsg( "Synchronizing EditorsData : OK" );
