...
#define GEOM_ENABLED(g) ((g) && ( (g)->gflags & GEOM_ENABLED ) )
...
//		return;
  ...
  for (dxGeom *g1=first; g1; g1=g1->next) 
  {
    ...
    if (GEOM_ENABLED(g1))
    {
    	if(!g1->body || dBodyIsEnabled(g1->body))
	    {
	      for (dxGeom *g2=g1->next; g2; g2=g2->next) 
	      {
			...
			if (GEOM_ENABLED(g2))
			{
			  if(g2->body && !dBodyIsEnabled(g2->body))
			  	continue;
...
void dxSimpleSpace::collide (void *data, dNearCallback *callback)
{
	...
	_pst_World = WOR_gpst_CurrentWorld;

	pst_FirstElem = TAB_pst_PFtable_GetFirstElem(&_pst_World->st_EOT.st_ODE);
	...
	{
		pst_A = (OBJ_tdst_GameObject *) pst_CurrentElem->p_Pointer;
		if(TAB_b_IsAHole(pst_A)) continue;

		if(!(pst_A->ul_IdentityFlags & OBJ_C_IdentityFlag_ODE)) 
			continue;
		
		pst_A_ODE = pst_A->pst_Base->pst_ODE;		
		
		if(!GEOM_ENABLED(pst_A_ODE->ode_id_geom)) 
			continue;		
		
		if(!(pst_A_ODE->uc_Flags & ODE_FLAGS_ENABLE))
			continue;

		AHasNoBody = !pst_A_ODE->ode_id_body;
		AIsPaused = AHasNoBody ? FALSE : !dBodyIsEnabled(pst_A_ODE->ode_id_body);

		pst_CurrentElem2 = pst_CurrentElem + 1;
		for(; pst_CurrentElem2 <= pst_EndElem; pst_CurrentElem2++)
		{
			pst_B = (OBJ_tdst_GameObject *) pst_CurrentElem2->p_Pointer;
			if(TAB_b_IsAHole(pst_B)) continue;
			
			if(!(pst_B->ul_IdentityFlags & OBJ_C_IdentityFlag_ODE)) 
				continue;
			
			pst_B_ODE = pst_B->pst_Base->pst_ODE;
						
			if(!GEOM_ENABLED(pst_B_ODE->ode_id_geom)) 
				continue;

			if(!(pst_B_ODE->uc_Flags & ODE_FLAGS_ENABLE))
				continue;

			BHasNoBody = !pst_B_ODE->ode_id_body;
			BIsPaused = BHasNoBody ? FALSE : !dBodyIsEnabled(pst_B_ODE->ode_id_body);

			if((AHasNoBody || AIsPaused) && (BHasNoBody || BIsPaused))
				continue;

			collideAABBs (pst_A_ODE->ode_id_geom, pst_B_ODE->ode_id_geom,data,callback);
		}
	}
...
static int64_t prime[NUM_PRIMES] = {1L,2L,3L,7L,13L,31L,61L,127L,251L,509L,
...
static ULONG getVirtualAddress (int level, int x, int y, int z)
{
...
void dxHashSpace::collide (void *data, dNearCallback *callback)
{
  ...
  maxlevel = global_minlevel - 1;
	...
	for(; pst_CurrentElem <= pst_EndElem; pst_CurrentElem++)
	{
		...
		if(!(pst_A_ODE->ode_id_geom) || !GEOM_ENABLED(pst_A_ODE->ode_id_geom))
			continue;

		geom = pst_A_ODE->ode_id_geom;

		dxAABB *aabb = (dxAABB*) o_Alloc.my_alloca (sizeof(dxAABB));
		aabb->geom = geom;
		...
		if (level <= global_maxlevel) 
		{
			...
			for (i=0; i < 6; i++) aabb->dbounds[i] = (int) floor (geom->aabb[i]/cellsize);
			// set AABB index
			aabb->index = n;
			n++;
		}
  ...
  unsigned char *tested = (unsigned char *) o_Alloc.my_alloca (n * tested_rowsize);
  ...
  Node **table = (Node **) o_Alloc.my_alloca (sizeof(Node*) * sz);
  ...
  for (aabb=first_aabb; aabb; aabb=aabb->next) 
  {
    ...
    for (int xi = dbounds[0]; xi <= dbounds[1]; xi++) 
	{
      for (int yi = dbounds[2]; yi <= dbounds[3]; yi++) 
	  {
		...
		{
		// get the hash index
		ULONG hi = getVirtualAddress (aabb->level,xi,yi,zi) % sz;
		// add a new node to the hash table
		Node *node = (Node*) o_Alloc.my_alloca (sizeof (Node));
		node->x = xi;
		node->y = yi;
		node->z = zi;
		node->aabb = aabb;
    ...
    for (int level = aabb->level; level <= maxlevel; level++) 
	{
      for (int xi = db[0]; xi <= db[1]; xi++) 
	  {
		for (int yi = db[2]; yi <= db[3]; yi++) 
		{
			...
			{
				// get the hash index
				ULONG hi = getVirtualAddress (level,xi,yi,zi) % sz;
