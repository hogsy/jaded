...
void dInternalHandleAutoDisabling (dxWorld *world, dReal stepsize)
{
	...
	for(; pst_CurrentElem <= pst_EndElem; pst_CurrentElem++)
	{
		...
		if(!pst_A_ODE->ode_id_body)
			continue;

		if(!(pst_A_ODE->uc_Flags & ODE_FLAGS_ENABLE))
			continue;

		bb = pst_A_ODE->ode_id_body;
		...
		idle = 1;			// initial assumption
		lspeed2 = dDOT(bb->lvel,bb->lvel);
	...
	for (bb=world->firstbody; bb; bb=(dxBody*)bb->next) {
		...
		pst_A = (OBJ_tdst_GameObject *) dGeomGetData(bb->geom);
		if(!pst_A) continue;

		if(!(pst_A->pst_Base->pst_ODE->uc_Flags & ODE_FLAGS_ENABLE))
			  continue;
...
void dxProcessIslands (dxWorld *world, dReal stepsize, dstepper_fn_t stepper)
{
  ...
  body = (dxBody**) o_Alloc.my_alloca (world->nb * sizeof(dxBody*));
  joint = (dxJoint**) o_Alloc.my_alloca (world->nj * sizeof(dxJoint*));
  ...
  dxBody **stack = (dxBody**) o_Alloc.my_alloca (stackalloc * sizeof(dxBody*));
	...
	for(; pst_CurrentElem <= pst_EndElem; pst_CurrentElem++)
	{
		...
		if(!pst_A->pst_Base->pst_ODE->ode_id_body) continue;
...
quickstart:
    ...
    // now do something with body and joint lists
	if(!pst_A || (pst_A->pst_Base->pst_ODE->uc_Flags & ODE_FLAGS_ENABLE))
	{
		stepper (world,body,bcount,joint,jcount,stepsize);
	}
