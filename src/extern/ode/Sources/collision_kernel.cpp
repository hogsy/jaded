...
void dGeomSetPositionNoDirty (dxGeom *g, dReal x, dReal y, dReal z)
{
  dAASSERT (g);
  dUASSERT (g->gflags & GEOM_PLACEABLE,"geom must be placeable");
  CHECK_NOT_LOCKED (g->parent_space);
  if (g->body) 
  {
    dBodySetPositionNoDirty (g->body,x,y,z);
  }
  else 
  {
    g->pos[0] = x;
...
void dGeomSetRotationNoDirty (dxGeom *g, const dMatrix3 R)
{
  dAASSERT (g && R);
  dUASSERT (g->gflags & GEOM_PLACEABLE,"geom must be placeable");
  CHECK_NOT_LOCKED (g->parent_space);
  ...
  {
    // this will call dGeomMoved (g), so we don't have to
    dBodySetRotationNoDirty (g->body,R);
  }
  else 
  {
...
void dGeomSetCategoryBits (dxGeom *g, ULONG bits)
{
  dAASSERT (g);
  CHECK_NOT_LOCKED (g->parent_space);
...
void dGeomSetCollideBits (dxGeom *g, ULONG bits)
{
  dAASSERT (g);
  CHECK_NOT_LOCKED (g->parent_space);
  g->collide_bits = bits;
...
ULONG dGeomGetCategoryBits (dxGeom *g)
{
  dAASSERT (g);
...
ULONG dGeomGetCollideBits (dxGeom *g)
{
  dAASSERT (g);
