...
#define TIM_MAIN_CLOCK_4TS TIM_gf_MainClock
#else
...
void MAT_UV_Compute_PLANAR_GIZMO(GDI_tdst_DisplayData	*pst_CurDD,GEO_tdst_Object	*pst_Obj,GEO_tdst_ElementIndexedTriangles	*pst_Element,ULONG CurrentAddFlag , ULONG StoreUV)
{
		...
		/* if (MAT_GET_MatrixFrom(pst_MLTTXLVL->s_AditionalFlags) == MAT_CC_WORLD) */
		{
			...
			case MAT_CC_X:
				{
					//TiZ Note: this will use the HW texture transformation for UV coord-gen
					MATH_tdst_Vector VU,VV,VW,VT,VS ;
					MATH_tdst_Matrix stM;
					MATH_InitVector(&VU , 0.f , 0.f , 1.f);
					MATH_InitVector(&VV , 1.f , 0.f , 0.f);
...
)
{
	...
	{
		if (Gx8_fneGetCurrentUVState() != Gx8_eUVOff)
		{
			MATH_tdst_Matrix Matrix;
            ...
            else if ( renderState_Shfited || ( Normalmap && RenderInNormal))
            {
			    Gx8_SetUVStageMatrix((D3DMATRIX *)&Matrix);
				// Transpose and Pass the transform matrix into the shader.
				{
					D3DXMATRIX SMatrix;
					D3DXMatrixTranspose(&SMatrix, &Matrix);
					IDirect3DDevice8_SetVertexShaderConstant(GX8_M_SD(GDI_gpst_CurDD)->mp_D3DDevice, 20, &SMatrix, 4);
				}
			}
		}
		return;
...

	{
		...
		if(!MAT_VUISIdentity_i(pst_MLTTXLVL))
		{
            ...
            else
            {
				if (Gx8_fneGetCurrentUVState() != Gx8_eUVOff)
				{
					...
					// Transpose and Pass the transform matrix into the shader.
					if ( renderState_Shfited || ( Normalmap && RenderInNormal) )
					{
						D3DXMATRIX SMatrix;
						D3DXMatrixTranspose(&SMatrix, &Matrix);
						IDirect3DDevice8_SetVertexShaderConstant(GX8_M_SD(GDI_gpst_CurDD)->mp_D3DDevice, 20, &SMatrix, 4);
				...
				{
					D3DXVECTOR4 p1={1,0,0,0},p2={0,1,0,0};
					//p1.x=1;p1.y=0;p1.z=0;p1.w=0;
					//p2.x=0;p2.y=1;p2.z=0;p2.w=0;
					IDirect3DDevice8_SetVertexShaderConstant(GX8_M_SD(GDI_gpst_CurDD)->mp_D3DDevice, 20, &p1, 1);
					IDirect3DDevice8_SetVertexShaderConstant(GX8_M_SD(GDI_gpst_CurDD)->mp_D3DDevice, 21, &p2, 1);
					Gx8_SetUVStageOff();
				}
            }
		}
		else
		{
			...
			{
				D3DXVECTOR4 p1={1,0,0,0},p2={0,1,0,0};//D3DXVECTOR4 p1,p2;
				//p1.x=1;p1.y=0;p1.z=0;p1.w=0;
				//p2.x=0;p2.y=1;p2.z=0;p2.w=0;
				IDirect3DDevice8_SetVertexShaderConstant(GX8_M_SD(GDI_gpst_CurDD)->mp_D3DDevice, 20, &p1, 1);
				IDirect3DDevice8_SetVertexShaderConstant(GX8_M_SD(GDI_gpst_CurDD)->mp_D3DDevice, 21, &p2, 1);
...
)
{
	...
	GDI_DrawIndexedTriangles(
				...
				pst_Obj->dst_PointNormal,//ajou yo
...
/*
 =======================================================================================================================
 =======================================================================================================================
	BEFORE CALLING MAT_DrawIndexedTriangle_MT the global variable g_MultipleVBIndex MUST BE SET TO THE VB INDEX TO USE
 */
...

{
...
#ifdef AFFICHEMATREDUIT
	static int sIDMAX=1000;
	static int sIDMIN=0;
#endif
	...
	if((pst_MLTTXLVL == NULL) || (!(pst_CurDD->ul_CurrentDrawMask & GDI_Cul_DM_UseTexture)))
	{
		...
		pst_CurDD->pst_CurrentMat = (MAT_tdst_MultiTexture *) _pst_Material;
			...
			// ENABLE TRILINEAR FILTERING (MIPMAP)
		IDirect3DDevice8_SetTextureStageState( GX8_M_SD(GDI_gpst_CurDD)->mp_D3DDevice, 0, D3DTSS_MIPMAPLODBIAS, *((LPDWORD) (&g_MipMapLODBias)));
		pst_CurDD->st_GDI.pfnv_SetTextureBlending((ULONG) - 1, MAT_Cul_Flag_Trilinear , 0);
//		pst_CurDD->st_GDI.pfnv_SetTextureBlending((ULONG) - 1, MAT_C_DefaultFlag, 0);
        
//	before GDI_DrawIndexedTriangles((*pst_CurDD),pst_Element,pst_CurDD->p_Current_Vertex_List,(GEO_tdst_UV *) p_CptBf->Current , pst_Obj->l_NbPoints);
	...
	else
	{
        ...
        {
/*yo            if(MAT_GET_s_Flags(pst_MLTTXLVL->s_AditionalFlags) & MAT_Cul_sFlag_DeductAlpha)
            {
                MAT_ComputeDeductedAlpha(pst_Obj, pst_MLTTXLVL);
                pst_CurDD->ul_DisplayInfo |= GDI_Cul_DI_UpdateRLI;
                //break;
            }
    */        pst_MLTTXLVL = pst_MLTTXLVL->pst_NextLevel;
        ...
        while (pst_MLTTXLVL && (LONG)pst_MLTTXLVL != -1)
		{
			if(!(pst_MLTTXLVL->ul_Flags & MAT_Cul_Flag_InActive))
			{
...
#ifdef AFFICHEMATREDUIT
				if ((pst_MLTTXLVL->s_TextureId>sIDMAX)||(pst_MLTTXLVL->s_TextureId<sIDMIN))
					break;
#endif
				...
				// Special xbox normal map !! -----
				if (MAT_GET_Blending(pst_MLTTXLVL->ul_Flags) == MAT_Cc_Op_Glow)//normal map
				{
					    Gx8_tdst_SpecificData   *pst_SD;
						pst_SD = GX8_M_SD(GDI_gpst_CurDD);

						Gx8_RS_UseTextureNormal(pst_SD,pst_MLTTXLVL->s_TextureId);
						RenderInNormal=true;
...
// TODO                    ERR_X_WarnOnce( false, "MAT_Cc_ColorOp_Diffuse2X not implemented on XBox" );
                    // fall through
				...
				if((!(pst_CurDD->ul_DisplayInfo & GDI_Cul_DI_RenderingDiffuse))&&(!(pst_CurDD->ul_DisplayInfo & GDI_Cul_DI_RenderingSpotSwadows)))
				{
					GDI_SetTextureBlending((*pst_CurDD),pst_MLTTXLVL->s_TextureId, pst_MLTTXLVL->ul_Flags | MAT_Cul_Flag_Trilinear, pst_MLTTXLVL->s_AditionalFlags);
				
					//Normal draw...i need to cumpute UV coordinates
				#ifdef GX8_BENCH
					if (!g_iNoUV)
				#endif
				...
				else
				{
					...
					if(pst_CurDD->ul_DisplayInfo & GDI_Cul_DI_RenderingDiffuse)
						blendingFlag=false;
					else
						blendingFlag=true;

					
					//NO UV COORDINATES CALCULATION IN THIS CASE

					//TO BE CAHNGED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					Gx8_SetTextureBlendingSBDiffuse(pst_MLTTXLVL->s_TextureId, pst_MLTTXLVL->ul_Flags | MAT_Cul_Flag_Trilinear, pst_MLTTXLVL->s_AditionalFlags,blendingFlag);
				}
				
/*yo				if(MAT_GET_s_Flags(pst_MLTTXLVL->s_AditionalFlags) & MAT_Cul_sFlag_DeductAlpha) 
					MAT_ComputeDeductedAlpha(pst_Obj, pst_MLTTXLVL);
*/
                ERR_X_Warning( !bIsSpriteGen, "Obsolete data: bIsSpriteGen, try to call MAT_RasterizeUV", 0 );

				if(!bIsSpriteGen)
					//before yo GDI_DrawIndexedTriangles((*pst_CurDD),pst_Element,pst_CurDD->p_Current_Vertex_List,(GEO_tdst_UV *) p_CptBf->Current , pst_Obj->l_NbPoints);
				GDI_DrawIndexedTriangles((*pst_CurDD),pst_Element,pst_CurDD->p_Current_Vertex_List,pst_Obj->dst_PointNormal,(GEO_tdst_UV *) p_CptBf->Current , pst_Obj->l_NbPoints);
					
						//For diffuse renedering...just one MT stage
						if((pst_CurDD->ul_DisplayInfo & GDI_Cul_DI_RenderingDiffuse)||(pst_CurDD->ul_DisplayInfo & GDI_Cul_DI_RenderingSpotSwadows))
							break;
			}

			pst_MLTTXLVL = pst_MLTTXLVL->pst_NextLevel;
#if defined(GX8_BENCH)
			if (g_iNoMulti) *(LONG *)&pst_MLTTXLVL = -1;
#endif			
		}
	}
	...
	//Call this one just if we are not rendering for shadow buffer stuff
	if((!(pst_CurDD->ul_DisplayInfo & GDI_Cul_DI_RenderingDiffuse))&&(!(pst_CurDD->ul_DisplayInfo & GDI_Cul_DI_RenderingSpotSwadows)))
		GDI_SetTextureBlending((*pst_CurDD), (ULONG) - 1, MAT_C_DefaultFlag, 0);
...
)
{
	...
	if(!GDI_gpst_CurDD->pst_CurrentGeo)
	{
		...
		while (pst_MLTTXLVL && (LONG)pst_MLTTXLVL != -1)
		{
			...
			if(!(pst_MLTTXLVL->ul_Flags & MAT_Cul_Flag_InActive))
			{
				...
				case MAT_Cc_ColorOp_FullLight:
					...
					break;
				case MAT_Cc_ColorOp_Diffuse2X:
// TODO                    ERR_X_WarnOnce( false, "MAT_Cc_ColorOp_Diffuse2X not implemented" );
                    // fall through
				...
				GDI_SetTextureBlending((*pst_CurDD),pst_MLTTXLVL->s_TextureId, pst_MLTTXLVL->ul_Flags, pst_MLTTXLVL->s_AditionalFlags);
	...
	GEO_tdst_ElementIndexedSprite	*pst_Element)
{
	...
	if(GDI_gpst_CurDD_SPR.pst_CurrentGeo)
	{
		GDI_gpst_CurDD_SPR.pst_CurrentGeo = pst_Obj;
	}
