...
#if 1
#define BZERO8_PTR(s,l,n)   L_memset(s,0,(lzo_uint)(l)*(n))
#else
#define BZERO8_PTR(s,l,n)   L_memset((lzo_voidp)(s),0,(lzo_uint)(l)*(n))
#endif
...
lzo_memcpy(lzo_voidp dest, const lzo_voidp src, lzo_uint len)
{
    ...
    return L_memcpy((char*)dest,(char*)src,len);
#else
...
lzo_memmove(lzo_voidp dest, const lzo_voidp src, lzo_uint len)
{
    ...
    return L_memmove((char*)dest,(char*)src,len);
#else
    lzo_byte *p1 = (lzo_byte *) dest;
    const lzo_byte *p2 = (const lzo_byte *) src;
...
static lzo_bool ptr_check(void)
{
	...
	for (i = 0; i < 10; i++)
	    dict[i] = wrkmem;
	BZERO8_PTR((void *) (dict+1),sizeof(dict[0]),8);
