...
#ifndef     _XEMATHFLOAT_H_
#define     _XEMATHFLOAT_H_
...
#define lInterpretDoubleAsLong(a)    (LONG )(*((long long *) &(a)))
...
_inline_ LONG lFloatToLongOpt(float a)
{
	...
	b = a + MATH_gd_Decal;
	return lInterpretDoubleAsLong(b);
}
...
Time:   Testé sur K6/2-333 fOptSqrt: 11 (cache ok) vOptSqrt 6-10 (cache ok, selon le contexte)
...
#if defined(_XENON)
_inline_ float fMin3(float a, float b, float c)
{
	float fmin = fMin(b, c);
	return fMin(a, fmin);
}
...
#if defined(_XENON)
_inline_ float fMax3(float a, float b, float c)
{
	float fmax = fMax(b, c);
	return fMax(a, fmax);
}
...
{
#if defined(_XENON)
	_fCoeff = fMax(_fCoeff, 0.0f);
	_fCoeff = fMin(_fCoeff, 1.0f);
#else
