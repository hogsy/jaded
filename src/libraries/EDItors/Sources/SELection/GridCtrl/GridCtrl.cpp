            ...
            if (m_bTitleTips)
            {
                ...
                if (pCell)
                {
                        ...
                        && GetCellRect(idCurrentCell.row, idCurrentCell.col, CellRect) )
                    {
						...
						m_TitleTip.Show(TextRect, pCell->GetTipText(), TextRect.Width(), CellRect,
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    {
        // - NOTE - Now done in the right-click method
...
void CGridCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if (m_MouseMode == MOUSE_NOTHING)
	{
		...
		if (cell.row >= m_nFixedRows && IsValid(m_LeftClickDownCell) && cell.col >= m_nFixedCols && bInTextArea)
		{
			OnEditCell(cell.row, cell.col, point, VK_RBUTTON);
		}
		else if (m_bListMode)
		{
			if (!IsValid(cell))
		...
		if (IsValid(cell))
		{
			BOOL bTrue = pCell->IsSelected();
			
			if (pCell)
				pCell->OnRClick(point);

			SendMessageToParent(cell.row, cell.col, NM_RCLICK);
		}
	}
...
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
    ...
    if (strCurrentText != str)
    {
        ...
        if (ValidateEdit(nRow, nCol, str) && SendMessageToParent(nRow, nCol, GVN_ENDLABELEDIT) >= 0)
        {
            SetModified(TRUE, nRow, nCol);
            RedrawCell(nRow, nCol);
        }
