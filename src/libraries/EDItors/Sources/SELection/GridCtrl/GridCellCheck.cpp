...
BOOL CGridCellCheck::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    // Do the draw 
	UINT uiState = m_bChecked ? DFCS_BUTTONCHECK | DFCS_CHECKED : DFCS_BUTTONCHECK ;
	uiState |= IsReadOnly() ? DFCS_INACTIVE : 0 ;

    pDC->DrawFrameControl(GetCheckPlacement(), DFC_BUTTON, uiState);
