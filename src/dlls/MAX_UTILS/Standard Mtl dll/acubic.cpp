...
//		ICustButton *iPick;
      ...
      //aszabo|Nov.15.01 - Drop method does not handle dropping NULL ref targets, so don't allow dropping them
		...
		void SetThing(ReferenceTarget *m) {
			ACubic *oldTex = theTex;
			theTex = (ACubic*)m;
//			ReloadDialog();
			if( oldTex!=theTex && oldTex!=NULL ) {
				for (int i=0; i<6; i++) StuffDlgName(i);
				StuffOutName();
				}
			}
...
class ACubicPathAccessor : public IAssetAccessor	{
public:

	ACubicPathAccessor(IParamBlock2* aBlock, int aTabIndex) : mBlock(aBlock), mTabIndex(aTabIndex) {}

	virtual int GetAssetType() const	{ return IAssetAccessor::kBitmapAsset; }
...
const TCHAR* ACubicPathAccessor::GetPath() const	{
	TCHAR* result;
	result = mBlock->GetStr(acubic_bitmap_names, 0, mTabIndex);
	return result;
}

void ACubicPathAccessor::SetPath(const TSTR& aNewPath)	{
	mBlock->SetValue(acubic_bitmap_names, 0, const_cast<TCHAR*>(aNewPath.data()), mTabIndex);
}

void ACubic::EnumAuxFiles(NameEnumCallback& nameEnum, DWORD flags)	{
	...
	if (useFile) {
		for (int i=0; i<6; i++) {
			if(flags & FILE_ENUM_ACCESSOR_INTERFACE)	{
				...
				callback->DeclareAsset(ACubicPathAccessor(pblock, i));
			}
			else	{
