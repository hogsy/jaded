...
void MultiDlg::UpdatePStamp( int i ) {
	HWND hPStamp  = GetDlgItem( hPanelBasic, mtlPStampIDs[i] );
	InvalidateRect(hPStamp, NULL, FALSE);
}
...
//		DeleteReference(i+1);
	...
	for (int i=0; i<subMtl.Count(); i++) {
		...
		ReferenceTarget* mtl = (ReferenceTarget*)GetStdMtl2Desc()->Create();
		hs.Resume();
		ReplaceReference(i+1, mtl);
...
float Multi::GetShinStr(int mtlNum, BOOL backFace) {
	...
	if(j<0)
		return 0.0f;
	if (subMtl[j]) return subMtl[j]->GetShinStr(mtlNum,backFace);
...
RefTargetHandle Multi::GetReference(int i) {
	if (loadingOld) {
		// russom - September 7, 2005 - 653392
		// Check index against size of subMtl
		if (i==0) 
			return NULL;
		else if ((i-1) < subMtl.Count() )
			return subMtl[i-1];
		else
			return NULL;
		}
