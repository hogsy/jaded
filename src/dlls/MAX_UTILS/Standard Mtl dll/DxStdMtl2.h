...
//	virtual LifetimeType	LifetimeControl() { return noRelease; }
	...
	LPD3DXEFFECT GetEffect(){return pEffect;}
...
class EffectElements
{
	...
	LPD3DXEFFECT pEffect;		//THe actual effect pointer
...
public:
	...
	inline int GetStyle(){return elementStyle;}
	...
	inline void SetDevice(LPDIRECT3DDEVICE9 dev){pDevice = dev;}
...
public:
	FloatElement(){};
	bool SetValue(LPD3DXEFFECT pEffect, StdMat2  * map);
...
public:
	...
	bool SetValue(LPD3DXEFFECT pEffect, IParamBlock2 * pblock);
	int GetLightType();
...
public:
	TransformElement();
	//void GetParamValidity(IParamBlock2 * pblock, TimeValue t, Interval & valid){ valid &= FOREVER;}
	bool SetValue(LPD3DXEFFECT pEffect, IParamBlock2 * pblock) {return true;}
