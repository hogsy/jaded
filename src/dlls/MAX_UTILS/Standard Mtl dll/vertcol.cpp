...
class __declspec(dllexport) NameData
{
public:
	int channel,subid;
};
...
//--------------------------------------------------------------
class __declspec(dllexport) VCol: public Texmap { 
			...
			for (int j = -1; j < 3; j++)
				...
				if (node->UserPropExists(key))
					...
					if (name.Length() > 0)
						{
						if (j == -1)
							name.printf("%d:%s : %s",i,GetString(IDS_ALL),name);
						else if (j == 0)
							name.printf("%d:%s : %s",i,GetString(IDS_RED),name);						
						else if (j == 1)
							name.printf("%d:%s : %s",i,GetString(IDS_GREEN),name);						
						else if (j == 2)
							name.printf("%d:%s : %s",i,GetString(IDS_BLUE),name);						
						NameData data;
						data.channel = i;
						data.subid = j;
