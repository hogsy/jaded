...
public:
	...
	__forceinline Point3(float X, float Y, float Z)  { x = X; y = Y; z = Z;  }
	__forceinline Point3(double X, double Y, double Z) { x = (float)X; y = (float)Y; z = (float)Z; }
	__forceinline Point3(int X, int Y, int Z) { x = (float)X; y = (float)Y; z = (float)Z; }
	__forceinline Point3(const Point3& a) { x = a.x; y = a.y; z = a.z; } 
	__forceinline Point3(float af[3]) { x = af[0]; y = af[1]; z = af[2]; }
