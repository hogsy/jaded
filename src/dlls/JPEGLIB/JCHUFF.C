...
typedef struct {
  struct Jade_jpeg_entropy_encoder pub; /* public fields */
...
start_pass_huff (j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (gather_statistics) {
      ...
      /* Note that Jade_jpeg_gen_optimal_table expects 257 entries in each table! */
	...
	entropy->ac_count_ptrs[actbl] = (LONG *)
      ...
      Jade_jpeg_make_c_derived_tbl(cinfo, cinfo->dc_huff_tbl_ptrs[dctbl],
			      & entropy->dc_derived_tbls[dctbl]);
      Jade_jpeg_make_c_derived_tbl(cinfo, cinfo->ac_huff_tbl_ptrs[actbl],
...
GLOBAL void
Jade_jpeg_make_c_derived_tbl (j_compress_ptr cinfo, JHUFF_TBL * htbl,
			 c_derived_tbl ** pdtbl)
{
  c_derived_tbl *dtbl;
  int p, i, l, lastp, si;
...
{
  struct Jade_jpeg_destination_mgr * dest = state->cinfo->dest;
		  ...
		  c_derived_tbl *dctbl, c_derived_tbl *actbl)
{
  ...
  for (k = 1; k < DCTSIZE2; k++) {
    if ((temp = block[Jade_jpeg_natural_order[k]]) == 0) {
      r++;
    } else {
...
GLOBAL void
Jade_jpeg_gen_optimal_table (j_compress_ptr cinfo, JHUFF_TBL * htbl, LONG freq[])
{
...
finish_pass_gather (j_compress_ptr cinfo)
{
  ...
  /* It's important not to apply Jade_jpeg_gen_optimal_table more than once
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
	...
	*htblptr = Jade_jpeg_alloc_huff_table((j_common_ptr) cinfo);
      Jade_jpeg_gen_optimal_table(cinfo, *htblptr, entropy->dc_count_ptrs[dctbl]);
      did_dc[dctbl] = TRUE;
    }
    if (! did_ac[actbl]) {
      ...
      if (*htblptr == NULL)
	*htblptr = Jade_jpeg_alloc_huff_table((j_common_ptr) cinfo);
      Jade_jpeg_gen_optimal_table(cinfo, *htblptr, entropy->ac_count_ptrs[actbl]);
      did_ac[actbl] = TRUE;
    }
  }
}
...
jinit_huff_encoder (j_compress_ptr cinfo)
{
  ...
  cinfo->entropy = (struct Jade_jpeg_entropy_encoder *) entropy;
