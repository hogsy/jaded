...
static inline void getClockCount (ULONG cc[2])
{
  LARGE_INTEGER a;
...
static inline double loadClockCount (ULONG cc[2])
{
  LARGE_INTEGER a;
  a.LowPart = cc[0];
  a.HighPart = cc[1];
...
//****************************************************************************
// implementation for Nintendo GameCube based on the GEKKO timer
...
static inline void getClockCount (ULONG cc[2])
{
  OSTick t;
  t = OSGetTick();
  cc[0] = OSTicksToMicroseconds(t);
  cc[1] = OSTicksToSeconds(t);
}
...
static inline double loadClockCount (ULONG a[2])
{
...
#if !defined(PENTIUM) && !defined(WIN32) && !defined(GCN) && !defined(_XBOX) && !defined(_XENON)
...
void dTimerReport (FILE *fout, int average)
{
  ...
  double *times = (double*) __ALLOCA (num * sizeof(double));
  for (i=0; i < (num-1); i++) {
