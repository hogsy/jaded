...
#define MAX_LINAR_SPEED 	10000
#define MAX_ANGULAR_SPEED 	10000


typedef const dReal *dRealPtr;
typedef dReal *dRealMutablePtr;
#define dRealArray(name,n) dReal name[n];
#define dRealAllocaArray(name,n) dReal *name = (dReal*) o_Alloc.my_alloca ((n)*sizeof(dReal));
	...
	dxQuickStepParameters *qs)
{
	...
	IndexError *order = (IndexError*) o_Alloc.my_alloca (m*sizeof(IndexError));
	...
	for (int iteration=0; iteration < num_iterations; iteration++) 
	{
		...
		for (int i=0; i<m; i++) 
		{
			...
			{
				fc_ptr = fc + 6*b2;
				delta -=fc_ptr[0] * J_ptr[6] + fc_ptr[1] * J_ptr[7] + fc_ptr[2] * J_ptr[8] + fc_ptr[3] * J_ptr[9] + fc_ptr[4] * J_ptr[10] + fc_ptr[5] * J_ptr[11];
			}
		     ...
		     dxJoint * const *_joint, int nj, dReal stepsize)
{
	...
	dxJoint **joint = (dxJoint**) o_Alloc.my_alloca(nj * sizeof(dxJoint*));
	...
	while(i--)
	{
		...
		dMULTIPLY2_333 (tmp,(*pbody)->mass.I,(*pbody)->R);
		dMULTIPLY0_333 (I+i*12,(*pbody)->R,tmp);
		// compute inverse inertia tensor in global frame
		dMULTIPLY2_333 (tmp,(*pbody)->invI,(*pbody)->R);
		dMULTIPLY0_333 (invI+i*12,(*pbody)->R,tmp);
		// compute rotational force
		dMULTIPLY0_331 (tmp,I+i*12,(*pbody)->avel);
		dCROSS ((*pbody)->tacc,-=,(*pbody)->avel,tmp);
	...
	while(i--)
	{
		if(!((*pbody)->flags & dxBodyNoGravity)) 
		{
			f1 = (*pbody)->mass.mass;
			(*pbody)->facc[0] += f1 * world->gravity[0];
			(*pbody)->facc[1] += f1 * world->gravity[1];
			(*pbody)->facc[2] += f1 * world->gravity[2];
		}
	...
	dxJoint::Info1 *info = (dxJoint::Info1*) o_Alloc.my_alloca (nj*sizeof(dxJoint::Info1));
	...
	int m = 0;
	int *ofs = (int*) o_Alloc.my_alloca (nj*sizeof(int));
	for (i=0; i<nj; i++) 
	{
		ofs[i] = m;
		m += info[i].m;
	...
	int *jb = (int*) o_Alloc.my_alloca (m*2*sizeof(int));
	if (m > 0) 
	{
		...
		int *findex = (int*) o_Alloc.my_alloca (m*sizeof(int));
		dSetZero (c,m);
	...
	for (i=0; i<nb; i++) 
	{
		...
		{			
            ...
            if(body[i]->lvel[j] > MAX_LINAR_SPEED)
            {
            	body[i]->lvel[j] = MAX_LINAR_SPEED;
            }
            else if(body[i]->lvel[j] < -MAX_LINAR_SPEED)
            {
            	body[i]->lvel[j] = -MAX_LINAR_SPEED;
            }
		...
		for (j=0; j<3; j++)
        {
            ...
            // Bug fixing : limit acceleration to avoid overflow of variables
            if (body[i]->tacc[j] > MAX_ACCELERATION)
            {
                body[i]->tacc[j] = MAX_ACCELERATION;
            }
            else if (body[i]->tacc[j] < -MAX_ACCELERATION)
            {
                body[i]->tacc[j] = -MAX_ACCELERATION;
            }
        ...
        {
            /* Angular Velocity THRESHOLD */
            if(body[i]->avel[j] > MAX_ANGULAR_SPEED)
            {
            	body[i]->avel[j] = MAX_ANGULAR_SPEED;
            }
            else if(body[i]->avel[j] < -MAX_ANGULAR_SPEED)
            {
            	body[i]->avel[j] = -MAX_ANGULAR_SPEED;
            }
        }
