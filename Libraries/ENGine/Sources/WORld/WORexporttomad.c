                               ...
                               BOOL _b_ExportSkin)
{
  ...
  if(st_MadWorld.NumberOfObjects)
  {
    ...
    for (ul = 0; ul < st_MadWorld.NumberOfObjects - ul_Waypoint; ul++)
    {
      ...
      if (_b_ExportSkin
        && pst_MadObj->IDType == ID_MAD_GeometricObject_V0
...
#else
		MAD_MALLOC(MAD_Dummy, ((MAD_Dummy *) pst_MadObj), 1);
#endif
  ...
  /* Save hierarchy nodes, add one to number to put root node in mad scene */
    ...
    if(st_MadWorld.NumberOfHierarchieNodes)
    {
        ...
        for(; pst_Elem <= pst_LastElem; pst_Elem++)
        {
			...
			if (pst_GO->sz_Name
			    && (pst_GO->sz_Name[0] == 'b' || pst_GO->sz_Name[0] == 'B')
				&& pst_GO->sz_Name[1] == '_')
			{
            ...
            if ( (pst_MadNode->Object != -1) && (pst_MadNode->Object < st_MadWorld.NumberOfObjects) )
            {
        ...
        if (_b_ExportSkin && st_MadWorld.NumberOfHierarchieNodes >= 3)
        {
          ...
          while (ul_HierarchyIdx > 0)
          {
            ...
            if (st_MadWorld.AllObjects[pst_MadNode->Object]->IDType == ID_MAD_SkinnedGeometricObject)
            {
    ...
    if (_b_ExportSkin)
    {
      for (ul = 0; ul < st_MadWorld.NumberOfObjects - ul_Waypoint; ul++)
      {
		    if (st_MadWorld.AllObjects[ul]->IDType == ID_MAD_SkinnedGeometricObject)
		    {
			    ...
			    MAD_PonderationList             *pst_Mad_PdrtList;
			    GEO_tdst_VertexPonderationList  *pst_Geo_PdrtList;
			    OBJ_tdst_GizmoPtr               *pst_GaO_GizmoPtr;
	        ...
	        pst_LastElem = TAB_pst_PFtable_GetLastElem(&_pst_World->st_AllWorldObjects);
			    ...
			    if (pst_GaO_Gro == pst_Gro)
			    {
				    pst_Object = (GEO_tdst_Object *)pst_Gro;
				    ...
				    for (j = 0; j < pst_SkndObj->us_NumberOfPonderationLists; ++j)
				    {
					    pst_Mad_PdrtList = &pst_SkndObj->pst_PonderationList[j];
					    pst_Geo_PdrtList = pst_Object->p_SKN_Objectponderation->pp_PdrtLst[j];
					    pst_GaO_GizmoPtr = pst_GaO->pst_Base->pst_AddMatrix->dst_GizmoPtr;

					    // Fill the pst_SkndObj.
					    if (pst_GaO_GizmoPtr[pst_Geo_PdrtList->us_IndexOfMatrix].pst_GO)
					    {
						    pst_Mad_PdrtList->us_MatrixIdx = 0;
						    psz_Ext = strrchr(pst_GaO_GizmoPtr[pst_Geo_PdrtList->us_IndexOfMatrix].pst_GO->sz_Name, '.');
						    if (psz_Ext)
							    *psz_Ext = 0;
						    while (pst_Mad_PdrtList->us_MatrixIdx < st_MadWorld.NumberOfHierarchieNodes
							    && strcmp(st_MadWorld.Hierarchie[pst_Mad_PdrtList->us_MatrixIdx].ID.Name, pst_GaO_GizmoPtr[pst_Geo_PdrtList->us_IndexOfMatrix].pst_GO->sz_Name))
					    ...
					    for (k = 0; k < pst_Mad_PdrtList->us_NumberOfPonderatedVertices; ++k)
					    {
