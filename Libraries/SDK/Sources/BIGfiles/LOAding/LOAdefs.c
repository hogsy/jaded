...
typedef struct	LOA_tdst_CallbackRasters_
{
	float	f_Current;
	float	f_Max;
	float	f_Min;
	...
	ULONG	pful_Callback;
	char	asz_Name[64];
} LOA_tdst_CallbackRasters;

#define M_InitRasterLine(_a)	extern ULONG _a(ULONG)
M_InitRasterLine(EVE_ul_CallbackLoadListTracks);
M_InitRasterLine(ANI_ul_ShapeCallback);
M_InitRasterLine(GEO_ul_Load_ObjectCallback);
M_InitRasterLine(OBJ_ul_GameObjectCallback);
M_InitRasterLine(OBJ_ul_World_LoadGolCallback);
M_InitRasterLine(COL_ul_CallBackLoadGameMaterial);
M_InitRasterLine(ACT_ul_ActionCallback);
M_InitRasterLine(AI_ul_CallbackLoadVars);
M_InitRasterLine(AI_ul_CallbackLoadFunction);
M_InitRasterLine(AI_ul_CallbackLoadModel);
M_InitRasterLine(WOR_ul_GameObjectGroupCallback);
M_InitRasterLine(COL_ul_CallBackLoadInstance);
M_InitRasterLine(WAY_ul_AllLinkListsCallback);
M_InitRasterLine(SND_ul_CallbackUnknownBank);
M_InitRasterLine(AI_ul_CallbackLoadInstance);
M_InitRasterLine(GRP_ul_LoadGrpCallback);
M_InitRasterLine(ACT_ul_ActionKitCallback);
M_InitRasterLine(COL_ul_CallBackLoadColSet);
#undef M_InitRasterLine
#define M_Size					22
#define M_InitRasterLine(_a)	{ 0.0f, 0.0f, 1000000.0f, 0.0f, 0.0f, (ULONG) _a, #_a }
LOA_tdst_CallbackRasters LOA_CallbackXRef[M_Size] =
{
	M_InitRasterLine(EVE_ul_CallbackLoadListTracks),
	M_InitRasterLine(ANI_ul_ShapeCallback),
	M_InitRasterLine(GEO_ul_Load_ObjectCallback),
...
void LOA_RegisterRaster(ULONG pCallback, float fstart, float fstop)
{
	/*~~~~~~~~~*/
	int		i;
	float	time;
	...
	for(i = 0; i < M_Size; i++)
	{
		if(LOA_CallbackXRef[i].pful_Callback == pCallback)
		{
			LOA_CallbackXRef[i].f_Hit++;
			LOA_CallbackXRef[i].f_Total += time;
			LOA_CallbackXRef[i].f_Current = time;
			LOA_CallbackXRef[i].f_Max = (LOA_CallbackXRef[i].f_Max > time) ? LOA_CallbackXRef[i].f_Max : time;
			LOA_CallbackXRef[i].f_Min = (LOA_CallbackXRef[i].f_Min < time) ? LOA_CallbackXRef[i].f_Min : time;
			return;
		}
	}
...
void LOA_PrintCBRaster(void)
{
	...
	total = 0.0f;
	printf("Total    Ave      Hit  Name\n");
	for(i = 0; i < M_Size - 1; i++)
	{
		if(!LOA_CallbackXRef[i].f_Hit) continue;
		total += LOA_CallbackXRef[i].f_Total;
		printf
		(
			"%f %f %4d %s\n",
			LOA_CallbackXRef[i].f_Total,
			LOA_CallbackXRef[i].f_Total / LOA_CallbackXRef[i].f_Hit,
			(int) LOA_CallbackXRef[i].f_Hit,
			LOA_CallbackXRef[i].asz_Name
		);
		LOA_CallbackXRef[i].f_Hit = 0.0f;
		LOA_CallbackXRef[i].f_Total = 0.0f;
		LOA_CallbackXRef[i].f_Current = 0.0f;
		LOA_CallbackXRef[i].f_Max = 0.0f;
		LOA_CallbackXRef[i].f_Min = 1000000.0f;
	}
