...
#define _breakpoint_	asm_break()
#else /* __CW__ */
#define _breakpoint_	asm __volatile__ ("break")
#endif /*__CW__ */


#define ERR_X_Assert(__Expr) if(!(__Expr)) \
		{ \
			printf("** %s(%d): Assert %s\n", __FILE__, __LINE__, #__Expr); \
    		_breakpoint_; \
		} \

#define ERR_X_ErrorAssert(__Expr, __T1, __T2) if(!(__Expr)) \
			printf("** %s(%d): ErrorAssert %s, %s\n", __FILE__, __LINE__, #__Expr, __T1); \

#define ERR_X_ForceAssert() printf("** %s(%d): ForceAssert.\n", __FILE__, __LINE__); \

#define ERR_X_Warning(_a, _b, _c) \
	if(!(_a)) printf("** %s(%d): Warning, %s\n", __FILE__, __LINE__, _b); 
...
#define ERR_X_Error(__Expr, __T1, __T2) if(!(__Expr)) \
		{ \
			/*printf("** %s(%d): Error %s, %s\n", __FILE__, __LINE__, #__Expr, __T1);*/ \
			_breakpoint_; \
		} \

#define ERR_X_ForceError(__T1, __T2) /*printf("** %s(%d): ForceError, %s\n", __FILE__, __LINE__, __T1)*/
...
#ifdef __cplusplus
extern "C" void GC_OutputDebugString(const char *_szFormat, ...);
extern "C" void GC_DisplayError(const char *_szMsg);
#else
extern void GC_OutputDebugString(const char *_szFormat, ...);
extern void GC_DisplayError(const char *_szMsg);
#endif /* __cplusplus */
...
#define _breakpoint_							OSHalt("BREAKPOINT\n");
/*#define ERR_X_Assert(a) 						ASSERT(a);
#define ERR_X_ErrorAssert(__Expr, __T1, __T2) 	ASSERT(__Expr);*/
#define ERR_X_Assert(__Expr) if(!(__Expr)) \
		{ char msg[256];\
		  sprintf(msg,"** %s(%d): ASSERTION : %s\n", __FILE__, __LINE__, #__Expr);\
	      GC_DisplayError(msg);\
		  OSHalt(msg);\
		}
...
#define ERR_X_ForceAssert() 					OSHalt("FORCE ASSERT\n");
#define ERR_X_Warning(__Expr, __T1, __T2) \
		if(!(__Expr)){ \
...
#define _breakpoint_ ERR_CallerBreak()
...
#define ERR_X_ErrorAssert(__Expr, __T1, __T2) \
		{ \
			...
			if(!(__Expr)) \
			{ \
				wsprintf (sBuf, "** %s(%d): ErrorAssert %s, %s\n", __FILE__, __LINE__, #__Expr, __T1); \
				xbERR_Print (sBuf); \
			} \
		} \
...
#define ERR_X_Warning(__Expr, __T1, __T2)       if (!(__Expr)) { }
#define ERR_X_Error(__Expr, __T1, __T2)         if (!(__Expr)) { }
#define ERR_X_ForceError(__T1, __T2)
