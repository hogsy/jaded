...
#	endif
					...
					else if ( pst_MLTTX->ul_ValidateMask & MAT_ValidateMask_Fur )
					{
...
#	endif
			...
			if ( pst_CurDD->ul_DisplayInfo & GDI_Cul_DI_ComputeShadowMap )
				*( LONG * ) &pst_MLTTXLVL = -1;
        ...
        GEO_tdst_ElementIndexedTriangles *pst_Element )
{
	...
	else if ( _pst_Material->st_Id.i->ul_Type == GRO_MaterialSingle )
	{
		PRO_IncRasterLong( &GDI_gpst_CurDD_SPR.pst_Raster->st_NbTriangles, pst_Element->l_NbTriangles );
