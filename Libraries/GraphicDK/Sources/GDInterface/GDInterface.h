...
typedef struct	GDI_tdst_Interface_
{
	...
	LONG	(*pfnl_OpenDisplay)				(void *);
#else
	LONG	(*pfnl_OpenDisplay)				(HWND, void *);
#endif
	LONG	(*pfnl_CloseDisplay)			(void *);
	LONG	(*pfnl_ReadaptDisplay)			(HWND, void *);
	void	(*pfnv_Clear)					(LONG, ULONG);
	void	(*pfnv_Flip)					(void);
	void	(*pfnv_SetViewMatrix)			(MATH_tdst_Matrix *);
	void	(*pfnv_SetProjectionMatrix)		(CAM_tdst_Camera *);
	LONG	(*pfnl_DrawIndexedTriangles)	(struct GEO_tdst_ElementIndexedTriangles_ *, GEO_Vertex *, GEO_tdst_UV *,ULONG);
	LONG	(*pfnl_Request)					(ULONG, ULONG);
#ifndef PSX2_TARGET
...
#endif    
    LONG    (*pfnl_InitTexture)             (struct GDI_tdst_DisplayData_ *, ULONG );
    void    (*pfnv_LoadTexture)             (struct GDI_tdst_DisplayData_ *, struct TEX_tdst_Data_ *, struct TEX_tdst_File_Desc_ *, ULONG);
    LONG    (*pfnl_StoreTexture)            (struct GDI_tdst_DisplayData_ *, struct TEX_tdst_Data_ *, struct TEX_tdst_File_Desc_ *, ULONG);
    ...
    void    (*pfnv_UnloadTexture)           (struct GDI_tdst_DisplayData_ *);
...
extern void Gx8_SetTextureBlending(ULONG _l_Texture, ULONG BM, unsigned short s_AditionalFlags);
#define GDI_DrawIndexedTriangles(GDIi,Tris,Vrtx,Normal, UV,ulNbPoints) 	Gx8_l_DrawElementIndexedTriangles(Tris , Vrtx , UV , ulNbPoints)
#define GDI_DrawIndexedSprites(GDIi,Tris,Vrtx,ulNbPoints) 	Gx8_l_DrawElementIndexedSprites	(Tris , Vrtx , ulNbPoints)
#define GDI_SetTextureBlending(GDIi,VA,VB,VC) 				Gx8_SetTextureBlending	(VA , VB , VC)
#define GDI_SetTextureTarget(GDIi,NST,CL) 				    Gx8_SetTextureTarget	(NST , CL )
#define GDI_SetViewMatrix(GDIi,TTMatrix) 				    Gx8_SetViewMatrix(TTMatrix)
#define GDI_SetViewMatrix_SDW(GDIi,MAT,Lim) 			    Gx8_SetViewMatrix_SDW	(MAT , Lim )

#elif defined(_XENON)
extern LONG Xe_l_DrawElementIndexedTriangles(GEO_tdst_ElementIndexedTriangles	*_pst_Element,GEO_Vertex*_pst_Point,GEO_tdst_UV*_pst_UV , ULONG);
extern LONG Xe_l_DrawElementIndexedSprites(GEO_tdst_ElementIndexedSprite	*_pst_Element,GEO_Vertex*_pst_Point,ULONG);
extern void Xe_SetViewMatrix(MATH_tdst_Matrix *);
extern void Xe_SetTextureBlending(ULONG _l_Texture, ULONG BM, unsigned short s_AditionalFlags);
#define GDI_DrawIndexedTriangles(GDIi,Tris,Vrtx,Normal, UV,ulNbPoints) 	Xe_l_DrawElementIndexedTriangles(Tris , Vrtx , UV , ulNbPoints)
#define GDI_DrawIndexedSprites(GDIi,Tris,Vrtx,ulNbPoints) 	Xe_l_DrawElementIndexedSprites	(Tris , Vrtx , ulNbPoints)
#define GDI_SetTextureBlending(GDIi,VA,VB,VC) 				Xe_SetTextureBlending	(VA , VB , VC)
#define GDI_SetTextureTarget(GDIi,NST,CL) 				Xe_SetTextureTarget	(NST , CL )
#define GDI_SetViewMatrix(GDIi,TTMatrix) 				Xe_SetViewMatrix(TTMatrix)
...
#else // PC + XBOX
#ifdef _XBOX
#define GDI_DrawIndexedTriangles(GDIi,Tris,Vrtx,Normals,UV,ulNbPoints) 	if (renderState_Shfited && !NoFUR) \
																		{ \
...
#define GDI_DrawIndexedTriangles(GDIi,Tris,Vrtx,Normals,UV,ulNbPoints) 	if (renderState_Shfited) \
...
#define GDI_gpst_CurDD_SPR gpst_GSP_stDD
#else
...
#if (defined(PSX2_TARGET) || defined(_GAMECUBE))
LONG					GDI_AttachDisplay(GDI_tdst_DisplayData *);
#else
...
#define                 GDI_b_IsXenonGraphics() TRUE
#else
