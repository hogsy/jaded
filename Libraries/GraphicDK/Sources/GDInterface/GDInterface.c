...
#ifdef PSX2_TARGET
#	include "GS_PS2/GSPtex.h"
#	include "GS_PS2/GSPinit.h"
#	include "GS_PS2/GSPrequest.h"
#	define GD_EXT GSP
#elif defined (_GAMECUBE)
#	include "GXI_GC/GXI_tex.h"
#	include "GXI_GC/GXI_init.h"
#	include "GXI_GC/GXI_request.h"
#elif (defined (ACTIVE_EDITORS) || defined (PCWIN_TOOL)) && !defined(_XENON_RENDER)
#	include <D3D8.h>
#	include "OpenGL/Sources/OGLtex.h"
#	include "OpenGL/Sources/OGLinit.h"
#	include "OpenGL/Sources/OGLrequest.h"
#	include "DX8/DX8tex.h"
#	include "DX8/DX8init.h"
#	include "DX8/DX8request.h"
#	define GD_EXT DX8
...
#	include "GX8/Gx8request.h" 
#	include "GX8/Gx8tex.h"
...
#	include "Dx9/Dx9tex.h"  
#	include "Dx9/Dx9request.h"  
#	include "Dx9/Dx9blending.h"  
...
#define M_INITGDI(GDI,DRV_EXT)\
    ...
    GDI->pfnl_OpenDisplay           = NULL;\
    GDI->pfnl_CloseDisplay          = NULL;\
    GDI->pfnl_ReadaptDisplay        = NULL;\
    ...
    GDI->pfnl_DrawIndexedSprites    = DRV_EXT##_l_DrawElementIndexedSprite;\
    GDI->pfnv_SetTextureBlending    = DRV_EXT##_SetTextureBlending;\
    GDI->pfnl_InitTexture			= DRV_EXT##_l_Texture_Init;\
    GDI->pfnv_LoadTexture           = DRV_EXT##_Texture_Load;\
    GDI->pfnl_StoreTexture          = NULL;/*DRV_EXT##_l_Texture_Store;*/\
...
#elif defined(_XENON) && !defined(_GX8)//popowarning
#define M_INITGDI(GDI,DRV_EXT)\
    GDI->pfnpv_InitDisplay          = (void * (*)(void)) DRV_EXT##_pst_CreateDevice;\
    GDI->pfnv_DesinitDisplay        = (void (__cdecl *)(void *))DRV_EXT##_DestroyDevice;\
    GDI->pfnl_OpenDisplay           = (LONG (__cdecl *)(HWND,void *))DRV_EXT##_l_Init;\
    GDI->pfnl_CloseDisplay          = (LONG (__cdecl *)(void *))DRV_EXT##_l_Close;\
    GDI->pfnl_ReadaptDisplay        = (LONG (__cdecl *)(HWND,void *))DRV_EXT##_l_ReadaptDisplay;\
    GDI->pfnv_Clear                 = DRV_EXT##_Clear;\
    GDI->pfnv_Flip                  = DRV_EXT##_RequestFlip;\
    GDI->pfnv_SetViewMatrix         = DRV_EXT##_SetViewMatrix;\
    GDI->pfnv_SetProjectionMatrix   = DRV_EXT##_SetProjectionMatrix;\
    GDI->pfnl_DrawIndexedTriangles  = DRV_EXT##_l_DrawElementIndexedTriangles;\
    GDI->pfnl_DrawIndexedSprites    = (void (__cdecl *)(GEO_tdst_ElementIndexedSprite_ *,MATH_tdst_Vector *,ULONG))DRV_EXT##_l_DrawElementIndexedSprites;\
    GDI->pfnl_Request               = DRV_EXT##_l_Request;\
    ...
    GDI->pfnv_UnloadTexture         = DRV_EXT##_Texture_Unload;\
    GDI->pfnv_SetTextureTarget      = DRV_EXT##_SetTextureTarget;\
    GDI->pfnv_SetViewMatrix_SDW		= (void(__cdecl*)(MATH_tdst_Matrix_*,float*))DRV_EXT##_SetViewMatrix_SDW;

//#elif defined( _XBOX ) || defined( ACTIVE_EDITORS ) || defined( PCWIN_TOOL ) || defined( _PC_RETAIL )
#elif defined( _XBOX ) || defined( ACTIVE_EDITORS ) || defined( PCWIN_TOOL ) || defined( _PC_RETAIL ) || (defined(_XENON) && defined(_GX8))
#ifdef JADEFUSION
#define M_INITGDI(GDI,DRV_EXT)\
    GDI->pfnpv_InitDisplay          = (void * (*)(void)) DRV_EXT##_pst_CreateDevice;\
    GDI->pfnv_DesinitDisplay        = (void (__cdecl *)(void *))DRV_EXT##_DestroyDevice;\
...
{
#ifdef PSX2_TARGET
	M_INITGDI(pst_GDI,GSP);
#elif defined(_GAMECUBE)
	M_INITGDI(pst_GDI,GXI);
#elif (defined(ACTIVE_EDITORS) || defined (PCWIN_TOOL)) && !defined(_XENON_RENDER_PC)

    if (_l_GDIType == 1)
    {
	    M_INITGDI(pst_GDI,DX8);
    }
    else
    {
    ...
    M_INITGDI(pst_GDI,Xe);
#elif defined(_XBOX) || (defined(_XENON) && defined(_GX8))
     M_INITGDI(pst_GDI,Gx8);
#elif defined(_PC_RETAIL)
    M_INITGDI(pst_GDI,Dx9);
#else
#   error TODO: Unsupported platform
#endif
    return 1;
...
void GDI_ChangeInterface(GDI_tdst_DisplayData *_pst_DD , ULONG ulNew)
{
	...
	_pst_DD->pv_SpecificData = _pst_DD->st_GDI.pfnpv_InitDisplay();
	_pst_DD->st_GDI.pfnl_OpenDisplay(_pst_DD->h_Wnd, _pst_DD);
#if defined(_XENON_RENDER_PC)
    if (_pst_DD->st_GDI.pfnv_Clear == Xe_Clear)
    {
...
ULONG GDI_GetInterface(GDI_tdst_DisplayData *_pst_DD )
{
	GDI_tdst_Interface *pst_GDI;
	pst_GDI = &_pst_DD->st_GDI;
	if (pst_GDI->pfnv_Clear == OGL_Clear)
		return 0;
#if defined(_XENON_RENDER_PC)
    else if (pst_GDI->pfnv_Clear == Xe_Clear)
        return 2;
#endif
	else
		return 1;
...
//#ifdef ACTIVE_EDITORS
	...
	if(DX8_gb_Init || (_pst_DD->st_TexManager.ul_Flags & TEX_Manager_ForceReload) )
#endif
	{
#if defined(ACTIVE_EDITORS) || defined(PCWIN_TOOL)
#ifdef JADEFUSION
...
void GDI_BeforeDisplay(GDI_tdst_DisplayData *_pst_DD)
{
    ...
    {
#ifndef _XENON // differed rendering, clear will be done later, just before draw, by the rendering thread
...
void GDI_AfterDisplay(GDI_tdst_DisplayData *_pst_DD)
{
...
#ifdef _XENON_RENDER
    _pst_DD->st_GDI.pfnv_Flip(); // always flip, the direct3d present frame will not happen to keep old frame on screen if not render or not flip is specified
    _pst_DD->ul_DisplayFlags &= ~GDI_Cul_DF_DoNotFlip; // always reset the flag (faster then testing if on)
    _pst_DD->ul_DisplayFlags &= ~GDI_Cul_DF_DoNotRender;
#else            
    /* flip */            
    if(!(_pst_DD->ul_DisplayFlags & (GDI_Cul_DF_DoNotRender | GDI_Cul_DF_DoNotFlip)) )
        _pst_DD->st_GDI.pfnv_Flip();
    else
        _pst_DD->ul_DisplayFlags &= ~GDI_Cul_DF_DoNotFlip;
#endif
    
#ifdef _GAMECUBE
    // If vidéo should start, start it here. It should start writing the first frame in copyXFB.
    GC_VideoFullScreenPlay();
#endif// _GAMECUBE
...
#ifdef ACTIVE_EDITORS
#ifdef JADEFUSION
	if ((_pst_DD->uc_SnapshotFlag & GDI_Cc_CaptureFlag_Running) 
		&& (_pst_DD->uc_SnapshotFlag & GDI_Cc_CaptureFlag_ForSeqBuilder))
	{
		//skip screenshot
...
#endif
	{
        ...
        st_BmpInfo.bmiHeader.biSizeImage = 0;
        st_BmpInfo.bmiHeader.biXPelsPerMeter = 0;
        st_BmpInfo.bmiHeader.biYPelsPerMeter = 0;
        st_BmpInfo.bmiHeader.biClrUsed = 0;
        st_BmpInfo.bmiHeader.biClrImportant = 0;
...
LONG GDI_l_DetachWorld( GDI_tdst_DisplayData *_pst_DD )
{
    ...
    _pst_DD->st_GDI.pfnv_UnloadTexture( _pst_DD );
...
OGL_TextureInformationLoad_StartAgain:
    ...
    if ( _pst_DD->st_GDI.pfnl_InitTexture( _pst_DD, l_NbTextures ) )
    {
        ...
        for( i = 0; i < l_NbTextures; i++, pst_Tex++, pst_CurTexDesc++)
	    {
            ...
            if 
			(
				(pst_CurTexDesc->st_Params.uc_Type == TEX_FP_RawPalFile)
...
void GDI_PrepareAllForXenonGraphics(void)
{
    if (GDI_sb_XenonGraphics)
        return;

    GDI_sb_XenonGraphics = TRUE;

    // Set the first level of the default material
    MAT_gst_DefaultSingleMaterial.pst_FirstLevel = &MAT_gst_DefaultMatMTLevel;

    // Assign a Xenon material to the default MT level
    MAT_gst_DefaultMatMTLevel.pst_XeMaterial = XeMaterial_CreateFromJade(&MAT_gst_DefaultSingleMaterial, 
                                                                         &MAT_gst_DefaultMatMTLevel);
}

void GDI_RestoreAllAfterXenonGraphics(void)
{
    if (!GDI_sb_XenonGraphics)
        return;

    GDI_sb_XenonGraphics = FALSE;

    // Default material
    if (MAT_gst_DefaultMatMTLevel.pst_XeMaterial != NULL)
    {
        XeMaterial_Destroy(MAT_gst_DefaultMatMTLevel.pst_XeMaterial);
        MAT_gst_DefaultMatMTLevel.pst_XeMaterial = NULL;
}
}
