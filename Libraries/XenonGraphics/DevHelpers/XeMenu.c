...
//#endif
	...
	float g_FrameRateAlarmLimit = 20.0f;
	...
	static Gsp_SetupMenu Gsp_Stp_SheetCodes =
	{
		"",
			0,0,0,
		{
			{"All full",0,0,-1,GSP_STP_ID_NULL,NULL,-1},
	...
	************************************************/
#if defined(_XENON)
	void Gsp_Stp_Draw(LPDIRECT3DDEVICE9 Device,Gsp_SetupMenu *p_Root , int X , int Y,int bIsSelection ,int bIsDefaultSelection)
#else
...
#define e_LeftTrigger INO_JoyMask_L
	...
	void Gsp_Setup(void *Dev)
	{
		...
		if (INO_b_Joystick_IsButtonJustDown(e_ButtonX))
		{
			if (INO_b_Joystick_IsButtonDown(e_ButtonB))
			{
				iPadActivated = 1;
			}
		}
		if (INO_b_Joystick_IsButtonJustDown(e_ButtonB))
		{
			if (INO_b_Joystick_IsButtonDown(e_ButtonX))
			{
				iPadActivated = 1;
			}
		}
	...
	*******************************************************************/
	void Gsp_Validate(Gsp_Stp_OneLeaf *p_OL , u_int Selection , u_int SetMenu)
	{
		...
		case GSP_STP_ID_MDF_ON:
			if (SetMenu)
			{
				p_OL -> Selection = g_iNoMDF; /* On */
			} else {
				g_iNoMDF = p_OL -> Selection; /* On */
			}
			break;
...
//			{"Pastel",0,0,0,GSP_STP_ID_CB_PASTEL,&Gsp_Stp_0_To_100,-1},
	 ...
	 Gsp_Stp_Video = (Gsp_SetupMenu)
	{
		...
		0,0,0,
		{
			...
			{"VIDEO 2D FFX",0,0,0,GSP_STP_ID_2DFFX,&Gsp_Stp_PlusMoin,-1},
			{"VIDEO 2D FFY",0,0,0,GSP_STP_ID_2DFFY,&Gsp_Stp_PlusMoin,-1},
			{"VIDEO 2D FFX Ofs",0,0,0,GSP_STP_ID_2DFFX_O,&Gsp_Stp_PlusMoin,-1},
			{"VIDEO 2D FFY Ofs",0,0,0,GSP_STP_ID_2DFFY_O,&Gsp_Stp_PlusMoin,-1},
			{"",0,0,0,0,NULL}
		}
	};
	 ...
	 Gsp_Stp_3D =(Gsp_SetupMenu)
	{
		...
		{
			{"2D",0,0,0,GSP_STP_ID_NO2D,&Gsp_Stp_OnOff,-1},
			{"Show normals",0,0,0,GSP_STP_ID_SHOWNORMALS,&Gsp_Stp_OnOff,-1},
	...
	Gsp_Stp_SoundFx = (Gsp_SetupMenu)
	{
		...
		0,0,0,
		{
			{"Enable0",0,0,0,GSP_STP_ID_SOUNDFXENABLE0,&Gsp_Stp_OnOff,-1},
			{"Mode0",0,0,0,GSP_STP_ID_SOUNDFXMODE0,&Gsp_Stp_SoundFxMode,-1},
			{"Delay0",0,0,0,GSP_STP_ID_SOUNDFXDELAY0,&Gsp_Stp_PlusMoin,-1},
			{"FeedBack0",0,0,0,GSP_STP_ID_SOUNDFXFEEDBACK0,&Gsp_Stp_PlusMoin,-1},
			{"WetPan0",0,0,0,GSP_STP_ID_SOUNDFXWETPAN0,&Gsp_Stp_PlusMoin,-1},
			{"WetVol0",0,0,0,GSP_STP_ID_SOUNDFXWETVOL0,&Gsp_Stp_PlusMoin,-1},
			{"",0,0,0,0,NULL}
		}
	};
...
WESET2:
		...
		case GSP_STP_ID_2DFFX:
			if (SetMenu)
			{
				p_OL -> Selection = 1;
				if (p_OL -> Open) 
					sprintf(Gsp_Stp_PlusMoin.Leaves[1].Name , "%.5f " , GspGlobal_ACCESS(f2DFFX_A2D));
			} else
			{
				if (Selection == 2) GspGlobal_ACCESS(f2DFFX_A2D)+= 0.00001f;
				if (Selection == 0) GspGlobal_ACCESS(f2DFFX_A2D)-= 0.00001f;
				GspGlobal_ACCESS(f2DFFX_A2D) = fMax(0.5f , fMin(10.5f , GspGlobal_ACCESS(f2DFFX_A2D)));
				STR_f_XFactor = 1.0f/(1.0f + (GspGlobal_ACCESS(f2DFFX_A2D) - 1.0f) / 1.0f);
				STR_f_XOffset = GspGlobal_ACCESS(Xsize) * (1.0f - STR_f_XFactor) / 1.8f;
				p_OL -> Selection = 1;
			}
			break;
		case GSP_STP_ID_2DFFY:
			if (SetMenu)
			{
				p_OL -> Selection = 1;
				if (p_OL -> Open) 
					sprintf(Gsp_Stp_PlusMoin.Leaves[1].Name , "%.4f " , GspGlobal_ACCESS(f2DFFY_A2D));
			} else
			{
				if (Selection == 2) GspGlobal_ACCESS(f2DFFY_A2D)+= 0.0001f;
				if (Selection == 0) GspGlobal_ACCESS(f2DFFY_A2D)-= 0.0001f;
				GspGlobal_ACCESS(f2DFFY_A2D) = fMax(0.5f , fMin(10.5f , GspGlobal_ACCESS(f2DFFY_A2D)));
...
//#define RONGING 40
		...
		case GSP_STP_ID_16_9:
			if (SetMenu)
			{
				if (GspGlobal_ACCESS(YCorrectionCoef) != (16.0f/9.0f) )
					p_OL -> Selection = 0; /* Off */
				else
					p_OL -> Selection = 1;
			} else {
				GspGlobal_ACCESS(YCorrectionCoef) = (5.5f/4.0f);
				if (Selection) GspGlobal_ACCESS(YCorrectionCoef) = (16.0f/9.0f);
			}
			break;
...
void GSP_Snapshot_Flush_Seq()
{
	...
	Gsp_Flush(FLUSH_ALL);	
	if (!(GspGlobal_ACCESS(FrameCounter) & 1))
	{ // Select buffer 2 
	...
	{
		Gsp_DrawRectangle(0.1f, 0.0f, 0.8f * (float)SeqCounter / (float)GSP_SequenBase , 0.1f  , 0xff , GST_COPY);
		Gsp_Flush(FLUSH_ALL);	
		sprintf(TgAName , "host0:x:/Snap/PS2_SEQ%05d.JPG" , NameCounter++);
...
void Gsp_Setup()
{
	...
	Gsp_Stp_OneLeaf *CurrentLeaf;
	{
		sprintf(Gsp_Stp_Root.Title ,"%s (%d)" , __DATE__, BIG_Cu4_AppVersion );
		if (TextureUsed)
