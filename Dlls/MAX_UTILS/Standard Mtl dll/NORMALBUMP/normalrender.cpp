...
DWORD Gnormal::GetActiveTexHandle(TimeValue t, TexHandleMaker& thmaker)
{
...
{
	if( (!map1on) || (!subTex[0]) )
		return (map2on && subTex[1] ? subTex[1]->EvalNormalPerturb(sc) : Point3(0,0,0));

	Point3 bp  =  (map2on && subTex[1]? subTex[1]->EvalNormalPerturb(sc) : Point3(0.0f,0.0f,0.0f));
	...
	case TANGENTUV_SPACE: {
			...
			p = ((p.x*tangentBasis.uBasis) + (p.y*tangentBasis.vBasis) + (p.z*sc.OrigNormal())).Normalize();
		}
		break;
	...
	if( gnMult!=1.0f )
	{
		Point3 oldNormal = sc.Normal();
		Point3 normalDif = p - oldNormal;
		p = (oldNormal + (gnMult*normalDif)).Normalize();
	}
	sc.SetNormal( p );
	return gnBMult*bp;
}
...
void Gnormal::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
DWORD LockCmpXChg( DWORD* p, DWORD* q, DWORD a, DWORD b ) {
	DWORD retval = TRUE;
	__asm {
		mov			eax, a
		mov			ebx, b
...
void ReaderWriterLock::EnterWrite() {
	DWORD checkBits, ok;
	while( TRUE ) {
		// If bits==0, sets bits=WRITER_BITS, otherwise ok=FALSE (atomic operation)
		ok = LockCmpXChg( &bits, &checkBits, 0, WRITER_BITS );
		if( ok ) break;
		else
			Sleep(0); //SwitchToThread();
	}
