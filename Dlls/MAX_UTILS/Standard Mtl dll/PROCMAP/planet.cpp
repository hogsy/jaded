...
namespace {
	...
	// [dl | 16jun2005] Fix to bug #666320. This fixes precision issues with very small
	// negative values. For example:
	// (double)1e-18 + 20.0 = 20.0	-- And this is wrong!
	// But with this fix:
	// (double)1e-18 + _nextafter(20, 0) = 19.9999999...
	const double slightlyLessThan_FNOISE_DIM = _nextafter(FNOISE_DIM, 0);
}
...
static float noise(float x, float y, float z) {
	int ix,iy,iz;
	float fx,fy,fz;
	double mx,my,mz;	// [dl | 08dec2004] Changed to double to fix precision issue
	float n,n00,n01,n10,n11,n0,n1;

	mx = fmod(x,FNOISE_DIM); if (mx<0) mx += slightlyLessThan_FNOISE_DIM;
	my = fmod(y,FNOISE_DIM); if (my<0) my += slightlyLessThan_FNOISE_DIM;
	mz = fmod(z,FNOISE_DIM); if (mz<0) mz += slightlyLessThan_FNOISE_DIM;
	ix = (int)mx;
	iy = (int)my;
	iz = (int)mz;
	...
	DbgAssert((noise_table != NULL) && (ix < NOISE_DIM) && (iy < NOISE_DIM) && (iz < NOISE_DIM));
...
//		paramDlg->pmap->SetParamBlock(pblock);
    ...
    RegisterDistanceDefault(_T("Planet Params"), _T("Size"), 40.0f, IN_TO_M(40.0f));
    float size = GetDistanceDefault(_T("Planet Params"), _T("Size"));
