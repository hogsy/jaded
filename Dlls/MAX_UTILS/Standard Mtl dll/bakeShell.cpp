...
class BakeShell : public Mtl, public IReshading  {	
	public:
		...
		void MappingsRequired(int subMtlNum, BitArray & mapreq, BitArray &bumpreq); // LAM - 6/15/04
...
// LAM - 6/15/04 - added
void BakeShell::MappingsRequired(int subMtlNum, BitArray & mapreq, BitArray &bumpreq) 
{
	int useMtl;
	mpBlock->GetValue( bakeShell_render_n_mtl, 0, useMtl, FOREVER );
	Mtl *mtl = (useMtl == 0 && mpOrigMtl) ? mpOrigMtl : mpBakedMtl;
	ULONG req = 0;
	if (mtl != NULL)
		mtl->MappingsRequired(subMtlNum,bumpreq,bumpreq);
}
