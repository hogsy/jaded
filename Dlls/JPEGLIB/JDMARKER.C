...
#define INPUT_VARS(cinfo)  \
	struct Jade_jpeg_source_mgr * datasrc = (cinfo)->src;  \
...
/* Process a SOFn marker */
{
  ...
  if (cinfo->comp_info == NULL)	/* do only once, even if suspend */
    cinfo->comp_info = (Jade_jpeg_component_info *) (*cinfo->mem->alloc_small)
			((j_common_ptr) cinfo, JPOOL_IMAGE,
			 cinfo->num_components * SIZEOF(Jade_jpeg_component_info));
...
/* Process a DQT marker */
{
  ...
  while (length > 0) {
    ...
    if (cinfo->quant_tbl_ptrs[n] == NULL)
      cinfo->quant_tbl_ptrs[n] = Jade_jpeg_alloc_quant_table((j_common_ptr) cinfo);
 ...
 *
 * Returns same codes as are defined for Jade_jpeg_consume_input:
...
GLOBAL boolean
Jade_jpeg_resync_to_restart (j_decompress_ptr cinfo, int desired)
{
...
jinit_marker_reader (j_decompress_ptr cinfo)
{
  ...
  cinfo->marker = (struct Jade_jpeg_marker_reader *)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_PERMANENT,
				SIZEOF(struct Jade_jpeg_marker_reader));
